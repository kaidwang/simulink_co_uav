...
#include <controller/impedance_control_node.h>
...
/***************************************************************************/
	...
	kp_f(0,0)=param.kf_p_x;
	kp_f(1,1)=param.kf_p_y;
	kp_f(2,2)=param.kf_p_z;
	kp_f(0,1)=kp_f(0,2)=kp_f(1,0)=
	kp_f(1,2)=kp_f(2,0)=kp_f(2,1)=0;

	ki_f(0,0)=param.kf_i_x;
	ki_f(1,1)=param.kf_i_y;
	ki_f(2,2)=param.kf_i_z;
	ki_f(0,1)=ki_f(0,2)=ki_f(1,0)=
	ki_f(1,2)=ki_f(2,0)=ki_f(2,1)=0;

	kd_f(0,0)=param.kf_d_x;
	kd_f(1,1)=param.kf_d_y;
	kd_f(2,2)=param.kf_d_z;
	kd_f(0,1)=kd_f(0,2)=kd_f(1,0)=
	kd_f(1,2)=kd_f(2,0)=kd_f(2,1)=0;

	kp_t(0,0)=param.kt_p_roll;
	kp_t(1,1)=param.kt_p_pitch;
	kp_t(2,2)=param.kt_p_yaw;
	kp_t(0,1)=kp_t(0,2)=kp_t(1,0)=
	kp_t(1,2)=kp_t(2,0)=kp_t(2,1)=0;

	ki_t(0,0)=param.kt_i_roll;
	ki_t(1,1)=param.kt_i_pitch;
	ki_t(2,2)=param.kt_i_yaw;
	ki_t(0,1)=ki_t(0,2)=ki_t(1,0)=
	ki_t(1,2)=ki_t(2,0)=ki_t(2,1)=0;

	kd_t(0,0)=param.kt_d_roll;
	kd_t(1,1)=param.kt_d_pitch;
	kd_t(2,2)=param.kt_d_yaw;
	...
	tao_cmd_wavy_star.resize(6), g_vector6.resize(6), v_wavy.resize(6);
	tao_cmd_wavy.resize(6), tao_com_wavy.resize(6);
	C.resize(6,6);
	...
	ref_thrust.x = ref_thrust.y = ref_thrust.z = 0;
	sensor_thrust.x = sensor_thrust.y = sensor_thrust.z = 0;
	ref_torque.x = ref_torque.y = ref_torque.z = 0;
	sensor_torque.x = sensor_torque.y = sensor_torque.z =0;
	//define a timer 
...
// }
	...
	ef(0) = ref_thrust.x-sensor_thrust.x;
	ef(1) = ref_thrust.y-sensor_thrust.y;
	ef(2) = ref_thrust.z-sensor_thrust.z;
...
void controller_class::calc_torques()
{
	...
	et(0) = ref_torque.x-sensor_torque.x;
	et(1) = ref_torque.y-sensor_torque.y;
	et(2) = ref_torque.z-sensor_torque.z;
...
void controller_class::output_publish(geometry_msgs::Point ang1,geometry_msgs::Point ang2,geometry_msgs::Point ang3,std_msgs::Float64 thu1,std_msgs::Float64 thu2,std_msgs::Float64 thu3)
{
	...
	thrust_value_pub.publish(thrust_value);
	torque_value_pub.publish(torque_value);
...
void controller_class::calc_cb(const ros::TimerEvent&)
{
	...
	{
		ROS_INFO_STREAM("normal impedance controller loop...");
