...
#include <controller/two_subcraft_linear_controller.h>
...
void linear_ava_controller::init_publisher()
{
	...
	// debug publish
	thrust_pub = nh.advertise<geometry_msgs::Point>("/thrust",10,this);
	torque_pub = nh.advertise<geometry_msgs::Point>("/torque",10,this);
}
...
{
	// ROS_INFO_STREAM("output_publish loop");
	// ROS_INFO_STREAM("ang1: "<<ang1);
	// ROS_INFO_STREAM("ang2: "<<ang2);
	// ROS_INFO_STREAM("ang3: "<<ang3);
	// ROS_INFO_STREAM("thu1: "<<thu1);
	// ROS_INFO_STREAM("thu2: "<<thu2);
	// ROS_INFO_STREAM("thu3: "<<thu3);
	...
	// debug publish
	thrust_pub.publish(thrust_debug);
	torque_pub.publish(torque_debug);
...
// 
	...
	B_full.block<1,6>(0,0) = B.block(0,0,1,6);
	B_full.block<1,6>(1,0) = B.block(1,0,1,6);
	B_full.block<1,6>(2,0) = B.block(2,0,1,6);
	B_full.block<1,6>(3,0) = B.block(4,0,1,6);
	B_full.block<1,6>(4,0) = B.block(5,0,1,6);
...
void linear_ava_controller::plant()
{
    ...
    if((thu1.data!=0)&&(thu2.data!=0)&&(!isnan(thu1.data))&&(!isnan(thu2.data)))
    {
    	// param_linear.composition(main_eular_angles,ang1,ang2,ang3,thu1,thu2,thu3,u_f3,u_e3);
		param_linear.two_composition(main_eular_angles,ang1,ang2,thu1,thu2,u_f3,u_e3);
...
void linear_ava_controller::calc_grad_phi()
{
	...
	ROS_INFO_STREAM(" err_xyz "<< err_xyz);
...
void linear_ava_controller::calc_U()
{
	...
	ua = Cau*Vu + ga + Ma*Va_d_dot +Ca*Va_d - kv*(Va-Va_d) - grad_phi.transpose() ;//- ki*sum_grad_phi_t; //check
	// ua = Cau*Vu + ga + Ma*Va_d_dot +Ca*Va_d - kv*(Va-Va_d) - grad_phi.transpose() - ki*sum_grad_phi_t; //check

	// ua = Cau*Vu + ga + Ma*Va_d_dot +Ca*Va_d - kv*(Va-Va_d) - grad_phi.transpose(); //check +ki*sum_ea
	uu = 0; // check
...
void linear_ava_controller::publish_main_pos()
{
    ...
    // ROS_INFO_STREAM("publish main_position: " << main_position);
	// ROS_INFO_STREAM("publish main_velocity: " << main_velocity);
	// ROS_INFO_STREAM("publish main_body_rates: " << main_body_rates);
    // ROS_INFO_STREAM("publish main_eular_angles: " << main_eular_angles);
    // // publish thrust 
