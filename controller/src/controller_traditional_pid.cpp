...
controller::controller(ros::NodeHandle* nodehandle):nh(*nodehandle)
{
	...
	kp_pos(0,1)=kp_pos(0,2)=kp_pos(1,0)=kp_pos(1,2)=kp_pos(2,0)=kp_pos(2,1)=0;
	//ROS_INFO_STREAM(kp_pos);

	ki_pos(0,0)=param.ki_x;
	ki_pos(1,1)=param.ki_y;
	ki_pos(2,2)=param.ki_z;
	ki_pos(0,1)=ki_pos(0,2)=ki_pos(1,0)=ki_pos(1,2)=ki_pos(2,0)=ki_pos(2,1)=0;

	kd_pos(0,0)=param.kd_x;
	kd_pos(1,1)=param.kd_y;
	kd_pos(2,2)=param.kd_z;
	kd_pos(0,1)=kd_pos(0,2)=kd_pos(1,0)=kd_pos(1,2)=kd_pos(2,0)=kd_pos(2,1)=0;
...
/***************************************************************************/
	...
	kp_att(0,1)=kp_att(0,2)=kp_att(1,0)=kp_att(1,2)=kp_att(2,0)=kp_att(2,1)=0;


	ki_att(0,0)=param.ki_phi;
	ki_att(1,1)=param.ki_theta;
	ki_att(2,2)=param.ki_psi;
	ki_att(0,1)=ki_att(0,2)=ki_att(1,0)=ki_att(1,2)=ki_att(2,0)=ki_att(2,1)=0;

	kd_att(0,0)=param.kd_phi;
	kd_att(1,1)=param.kd_theta;
	kd_att(2,2)=param.kd_psi;
	kd_att(0,1)=kd_att(0,2)=kd_att(1,0)=kd_att(1,2)=kd_att(2,0)=kd_att(2,1)=0;
	...
	//define a timer 
    calc_timer = nh.createTimer(ros::Duration(detla_time), &controller::calc_cb, this);  //timer used to publish state, should be at least for some minimal frequency
...
void controller::init_publisher()
{
...
//subscriber function
void controller::init_subscriber()
{
	...
	*/
	...
	("/nominal_position",1,&controller::nominal_position_sub_cb,this);
	nominal_eular_angles_sub = nh.subscribe<geometry_msgs::Point>
	("/nominal_euler_angles",1,&controller::nominal_eular_angles_sub_cb, this);
	main_position_sub = nh.subscribe<geometry_msgs::Point>
	("/main_position",1,&controller::main_position_sub_cb,this);
	main_velocity_sub = nh.subscribe<geometry_msgs::Point>
	("/main_velocity",1,&controller::main_velocity_sub_cb,this);
	main_eular_angles_sub = nh.subscribe<geometry_msgs::Point>
	("/main_euler_angles",1,&controller::main_eular_angles_sub_cb,this);
	main_body_rates_sub = nh.subscribe<geometry_msgs::Point>
	("/main_body_rates",1,&controller::main_body_rates_sub_cb,this);
	init_euler_angles_cmd_sub = nh.subscribe<geometry_msgs::Point>
	("/init_euler_angles_cmd",1,&controller::init_euler_angles_cmd_sub_cb,this);
	init_pos_cmd_sub = nh.subscribe<geometry_msgs::Point>
	("/init_position_cmd",1,&controller::init_pos_cmd_sub_cb,this);
	init_body_rates_cmd_sub = nh.subscribe<geometry_msgs::Point>
	("/init_body_rates_cmd",1,&controller::init_body_rates_cmd_sub_cb,this);
	init_velocity_cmd_sub = nh.subscribe<geometry_msgs::Point>
	("/init_velocity_cmd",1,&controller::init_velocity_cmd_sub_cb,this);
	control_start_sub_att = nh.subscribe<std_msgs::Bool>
	("/start_pub_att",1,&controller::control_start_sub_att_cb,this);
	
	//kaidi wang, 2021.10.13, mode_switch_sub define
	mode_switch_sub       = nh.subscribe<std_msgs::Bool>
	("/mode_switch",1,&controller::mode_switch_sub_cb,this);

	psi_bias_1_sub = nh.subscribe<std_msgs::Float64>("/psi_bias_1",1,&controller::psi_bias_1_sub_cb,this);
	psi_bias_2_sub = nh.subscribe<std_msgs::Float64>("/psi_bias_2",1,&controller::psi_bias_2_sub_cb,this);
	psi_bias_3_sub = nh.subscribe<std_msgs::Float64>("/psi_bias_3",1,&controller::psi_bias_3_sub_cb,this);
...
//top level function
void controller::controller_body()//controller body
{
...
//controller body
void controller::control_allocation()
{
	ctrl_allocation();
}
...
void controller::ctrl_allocation()
{
	Eigen::Matrix3f I;
	I(0,0)=I(1,1)=I(2,2)=1;
	...
	//allocation function
	double psi_cmd1=nominal_euler_angles.z+node1_yaw.data;
	double psi_cmd2=nominal_euler_angles.z+node2_yaw.data;
	double psi_cmd3=nominal_euler_angles.z+node3_yaw.data;
...
Eigen::Vector4f controller::alloc(Eigen::Vector3f f,double psi_cmd)
{
...
void controller::rotation_kine_inerse()//kinematic inverse of rotation
{
...
void controller::trans_kine_inverse()//translational kinematic inverse
{
	//calc dot 
...
void controller::calc_epsilon_v()
{
...
void controller::calc_epsilon_w()
{
...
double controller::differentiator
(
	double u, 
...
void controller::RK4_02(double u,double omega,double epsilon,double *x,double *y,double *z,double h)
{
...
void controller::differentiator_lastest(double *last,double *now,double *re,double h)
{
    *re = (*now-*last)/h;
}

void controller::kinematics_pseudoinverse
...
//calc_vd chk
void controller::calc_vd(double xd_dot, double yd_dot, double zd_dot)
{
...
//gain_1 function chk
double controller::gain_1(double u,double omega)//chk
{
	double y;
	y=omega*omega*u;
	return y;
...
//gain_2 function chk
double controller::gain_2(double u,double omega,double epsilon)
{
	double y;
	y=2*omega*epsilon*u;
	return y;
...
Eigen::Matrix3f controller::euler_to_rotation_mat(geometry_msgs::Point angle)
{
	Eigen::Matrix3f mat;
...
double controller::d2f(double u,double omega,double epsilon,double x,double y,double z)
{
	double f2;
	f2=u-2*omega*epsilon*z-omega*omega*y;
	return f2;
}
double controller::df(double x,double y,double z)
{
	double f1;
	f1=z;
	return f1;
...
//other function
Eigen::Matrix3f controller::pos_mat(double x,double y,double z)
{
	Eigen::Matrix3f s;
	s(0,0)=s(1,1)=s(2,2)=0;
	s(0,1)=-z;
...
void controller::nominal_position_sub_cb(const geometry_msgs::Point::ConstPtr& msg)
{
...
void controller::nominal_eular_angles_sub_cb(const geometry_msgs::Point::ConstPtr& msg)
{
...
//position back, chk
void controller::main_position_sub_cb(const geometry_msgs::Point::ConstPtr& msg)
{
	main_position = *msg;
...
//velocity back, chk
void controller::main_velocity_sub_cb(const geometry_msgs::Point::ConstPtr& msg)
{
	main_velocity = *msg;
...
//attitude back, chk
void controller::main_eular_angles_sub_cb(const geometry_msgs::Point::ConstPtr& msg)
{
...
//body rates back,chk
void controller::main_body_rates_sub_cb(const geometry_msgs::Point::ConstPtr& msg)
{
	main_body_rates = *msg;
...
//init attitude,chk
void controller::init_euler_angles_cmd_sub_cb(const geometry_msgs::Point::ConstPtr& msg)
{
...
//init position,chk
void controller::init_pos_cmd_sub_cb(const geometry_msgs::Point::ConstPtr& msg)
{
	init_position = *msg;
	// ROS_INFO_STREAM("init_position: ");
	// ROS_INFO_STREAM(init_position);
...
//init body rate,chk
void controller::init_body_rates_cmd_sub_cb(const geometry_msgs::Point::ConstPtr& msg)
{
	init_body_rate = *msg;
...
//init velocity 
void controller::init_velocity_cmd_sub_cb(const geometry_msgs::Point::ConstPtr& msg)
{
	init_velocity = *msg;
...
void controller::mode_switch_sub_cb(const std_msgs::Bool::ConstPtr& msg)
{
	mode_switch = *msg;
}

//control start chk
void controller::control_start_sub_att_cb(const std_msgs::Bool::ConstPtr& msg)
{
	start_pub_att = *msg;
}
//psi_bias 1 chk
void controller::psi_bias_1_sub_cb(const std_msgs::Float64::ConstPtr& msg)
{
	node1_yaw = *msg;
}
//psi_bias 2 chk
void controller::psi_bias_2_sub_cb(const std_msgs::Float64::ConstPtr& msg)
{
	node2_yaw = *msg;
}
//psi_bias 3 chk
void controller::psi_bias_3_sub_cb(const std_msgs::Float64::ConstPtr& msg)
{
	node3_yaw = *msg;
}

//member function list
void controller::output_publish(geometry_msgs::Point ang1,geometry_msgs::Point ang2,geometry_msgs::Point ang3,std_msgs::Float64 thu1,std_msgs::Float64 thu2,std_msgs::Float64 thu3)
{
...
geometry_msgs::Point controller::range_judge(geometry_msgs::Point origin, double range)
{
...
Eigen::Vector3f controller::calc_vd_p_controller(Eigen::Matrix3f p, 
...
void controller::time_run()
{
	//count cycle times
	run_times++;
}
//timer callback function
void controller::calc_cb(const ros::TimerEvent&)
{
	//ROS_INFO_STREAM("kaidi gaoling love ...");
...
int main(int argc, char **argv)
{
	...
	// dynamic_reconfigure::Server<controller_config::paramConfig> server;
	...
	controller controller_node(&nh);//init some param and then start the controller 
	ros::spin();
	return 0;
}
