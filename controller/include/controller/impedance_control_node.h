...
private:
    ...
    Eigen::Matrix3f kp_f;      //p gain of position
    Eigen::Matrix3f ki_f;      //i gain of position
    Eigen::Matrix3f kd_f;      //d gain of position

    Eigen::Matrix3f kp_t;      //p gain of attitude
    Eigen::Matrix3f ki_t;
    Eigen::Matrix3f kd_t;
    ...
    geometry_msgs::Point thrust_value;
    geometry_msgs::Point torque_value;
    //input list
    ...
    ros::Publisher thrust_value_pub;
    ros::Publisher torque_value_pub;
    //subscriber list
    //geometry_msgs::Point
    ...
    ros::Subscriber ref_thrust_sub;
    ros::Subscriber ref_torque_sub;
    ros::Subscriber sensor_thrust_sub;
    ros::Subscriber sensor_torque_sub;
    ...
    void ref_thrust_sub_cb(const geometry_msgs::Point::ConstPtr& msg );
    void ref_torque_sub_cb(const geometry_msgs::Point::ConstPtr& msg);
    void sensor_thrust_sub_cb(const geometry_msgs::Point::ConstPtr& msg );
    void sensor_torque_sub_cb(const geometry_msgs::Point::ConstPtr& msg);
...
// impedance controller define 
private:

    geometry_msgs::Point ref_thrust; // ref thrust value
    geometry_msgs::Point ref_torque; // ref torque value
    geometry_msgs::Point sensor_thrust; // sensor thrust value
    geometry_msgs::Point sensor_torque; // sensor torque value

    Eigen::Vector3f ef,ef_init, ef_ls, d_ef;
    Eigen::Vector3f et,et_init, et_ls, d_et;

    Eigen::Vector3f f_dir;
    Eigen::VectorXf tao_dir_wavy, tao_imp_wavy;
    Eigen::VectorXf tao_ext_wavy_hat;
    Eigen::VectorXf ep_wavy, ev_wavy;
    Eigen::VectorXf v_wavy;

    Eigen::VectorXf tao_cmd_wavy_star, tao_cmd_wavy, tao_com_wavy, g_vector6;

    Eigen::Matrix3f Kfp,Kfi;
    Eigen::MatrixXf II6;
    Eigen::MatrixXf Mv,Dv,Kv;
    Eigen::MatrixXf K_I,M;
    Eigen::VectorXf inter_tao_ext;
    Eigen::VectorXf inter_tao_ext_init, inter_tao_ext_ls;

    Eigen::Vector3f inter_tao_ext_ls_h3, inter_tao_ext_ls_t3;
    ...
    // new impedance controller 
    Eigen::Vector3f thrust_impedance_controller(geometry_msgs::Point f_e, geometry_msgs::Point f);
    Eigen::Vector3f torque_impedance_controller(geometry_msgs::Point t_e, geometry_msgs::Point t);

    
    void calc_f_dir();
    void calc_tao_imp_wavy();
    void calc_tao_ext_wavy_hat();
    void calc_tao_cmd_wavy();
    void impedance_controller();
    Eigen::Vector3f integrator_vector3f(Eigen::Vector3f input,Eigen::Vector3f orign,Eigen::Vector3f *ls,double t);
    void differentiator_lastest_vector3f(Eigen::Vector3f *last,Eigen::Vector3f *now,Eigen::Vector3f *re,double h);
