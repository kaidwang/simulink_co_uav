...
// sensor output var
geometry_msgs::Point main_pos_ned ;//= 0;
geometry_msgs::Point main_velocity_ned ;//= 0;
geometry_msgs::Point main_eular_angles_ned ;//= 0;
geometry_msgs::Point main_body_rates_ned ;//= 0;
...
int sw_kill=0;
int thrust=0;
int switch_yaw_xyz[4]={0,0,0,0};//xyz,yaw
...
int init_data(geometry_msgs::Point init_pos,geometry_msgs::Point init_att)
{
    int ret=1;
    pos_data.x =init_pos.x;
    pos_data.y =init_pos.y;
    pos_data.z =init_pos.z;//6.1234515;
    euler_data.x = init_att.x;//0.0015215;
    euler_data.y = init_att.y;//-0.0021456;
    euler_data.z = 0;//init_att.z;//1.400254;
    ...
    main_pos_ned.x = main_pos_ned.y =main_pos_ned.z =0;
	main_velocity_ned.x= main_velocity_ned.y = main_velocity_ned.z =0;
	main_eular_angles_ned.x= main_eular_angles_ned.y = main_eular_angles_ned.z = 0;
	main_body_rates_ned.x = main_body_rates_ned.y = main_body_rates_ned.z = 0;
...
{
	get_rc_channel = *msg;
	sw_arm = get_rc_channel.channels[6]; // arm switch strick
	// sw_auto_land = get_rc_channel.channels[5];
	sw_kill = get_rc_channel.channels[7]; // kill switch strick
	...
	// ROS_INFO_STREAM("channel 7: "<<get_rc_channel.channels[6]<<" "<<sw_arm);
    // ROS_INFO_STREAM("channel 8: "<<get_rc_channel.channels[7]<<" "<<sw_kill);
	// ROS_INFO_STREAM("channel 9: "<<get_rc_channel.channels[8]);
	// ROS_INFO_STREAM("channel 5: "<<get_rc_channel.channels[4]<<" "<<switch_position_attitude);
}
...
int main(int argc, char **argv)
{
    ...
    // define arm or kill switch param
    std_msgs::Bool arm_bool;
    std_msgs::Bool autoland_bool;
    std_msgs::Bool normal_bool;
    arm_bool.data =false;//arm
    autoland_bool.data = false;//land and stop or kill directly
    normal_bool.data = false;//armed and not fly state
    ...
    geometry_msgs::Point init_attitude;
    ...
    // // wait 200 times get position and attitude data
    // while (ros::ok())
    // {

    //     for (int i = 0; i < 200; i++)
    //     {
    //         ROS_INFO_STREAM("get some position and attitude data..");
    //         init_position = enu2ned_pos(make_main_position(local_pos));
    //         init_attitude = enu2ned_euler(make_eular_angles(local_pos));
    //         ros::spinOnce();
	// 		rate.sleep();
    //     }
    //     break;
    ...
    int init = init_data(init_position, init_attitude);
    ...
    while (ros::ok()&&outloop_tri.data == false)
    {
        ...
        ROS_INFO_STREAM("arm and init all control data..");
    ...
    while (ros::ok())
    {
		...
		// }
		// else if((sw_kill > 1500 && sw_arm > 1500)||(sw_auto_land > 1500 && sw_arm < 1500))
		// {
		// 	//take off switch
		...
		// 	normal_bool.data = false;
		// }
		// else if(sw_kill < 1500 && sw_arm < 1500)
		// {
		// 	//normal crc switch
		...
		// }
        // // judge if the thrust strick is up to middle
		...
		// 	count ++ ;// during the flight, avoid repeating the initialization after returning to the radio middle
        ...
        //     // pos_data = enu2ned_pos(make_main_position(local_pos));
        //     // euler_data = enu2ned_euler(make_eular_angles(local_pos));
        //     // vel_data = enu2ned_vel(make_main_velocity(local_vel));
        //     // body_rate_data = enu2ned_euler_rate(make_main_body_rates(local_vel));
        ...
        //     // nominal_position_pub.publish(pos_data);
        //     // nominal_eular_angles_pub.publish(euler_data);
        //     // nominal_velocity_pub.publish(vel_data);
        //     // nominal_body_rate_pub.publish(body_rate_data);
        //     }
        // }
        // else
        // {
        //     // check wether the init pos data is changed
        //     ROS_INFO_STREAM("pos_data:"<<pos_data);
        ...
        //     // position_output=calc_circle(radius, x_center, y_center,
        ...
        //     // publish position, attitude, velocity, body_rate, ned frame
        //     publish_sensor_data();
