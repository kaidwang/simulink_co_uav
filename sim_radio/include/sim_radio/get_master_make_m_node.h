...
#include <sim_radio/param_list.h>//include the param list 
#include <fstream>//output log file
#include <math.h>

// define a flag of real or simulation test
int flag_real_sim = 0;
...
private:
    ...
    float deceleration_time   = 10;    //unit s
...
public:
    ...
    geometry_msgs::Point vel_setpoint;//setpoint velocity
    geometry_msgs::Point body_rate_setpoint;//setpoint body rate

    // switch value to linear velocity
    void sw_map_vel(int* sw)
    {
        // channel 1 map section
        ...
        {
            vel[0]=((float)(sw[0]-channel1_mid)/(float)(channel1_up-channel1_mid))*yaw_speed;//channel 1
        }
        // channel 2 map section
        ...
        {
            vel[3]=-((float)(sw[3]-channel4_mid)/(float)(channel4_up-channel4_mid))*fly_speed;//channel 4        
        }
        ...
        // }
        vel_setpoint.x = vel[1];
        vel_setpoint.y = vel[3];
        vel_setpoint.z = vel[2];
        body_rate_setpoint.x=0;
        body_rate_setpoint.y=0;
        body_rate_setpoint.z=vel[0];
    }

    // body rate map section
    void sw_map_ang_vel(int* sw)
    {
        ...
        {
        	ang_vel[0]=((float)(sw[0]-channel1_mid)/(float)(channel1_up-channel1_mid))*yaw_speed;//channel 1
        }
        // channel 2 map section       
        if((float)(abs(sw[1]-channel2_mid))<=15)
        {
        ...
        {
                ang_vel[2]=-((float)(sw[3]-channel4_mid)/(float)(channel4_up-channel4_mid))*angular_velocity;//channel 3
        }
        vel_setpoint.x = 0;
        vel_setpoint.y = 0;
        vel_setpoint.z = ang_vel[3];
        body_rate_setpoint.x=ang_vel[2];
        body_rate_setpoint.y=ang_vel[1];
        body_rate_setpoint.z=ang_vel[0];
    }
    ...
    (geometry_msgs::Point pos_last, geometry_msgs::Point att_last,float *ang_vel,float t)
    {
	    ...
	    att_setpoint.x = att_last.x + ang_vel[2]*t;
	    att_setpoint.y = att_last.y + ang_vel[1]*t;
        att_setpoint.z = att_last.z + ang_vel[0]*t;
         //ROS_INFO_STREAM("yaw: "<<att_last.z);

    } 
    //keep change roll angle
/*    
void keep_change_roll_pitch_angle(geometry_msgs::Point att_last, float t)
    {
    ...
    void back_roll_pitch_to_origin(geometry_msgs::Point init_euler_angles,geometry_msgs::Point att_last, float t)
    {
...
sw_mapping::sw_mapping(/* args */)
{
    ...
    vel_setpoint.x = vel_setpoint.y = vel_setpoint.z = 0;//setpoint velocity
    body_rate_setpoint.x = body_rate_setpoint.y = body_rate_setpoint.z = 0;//setpoint body rate
}
